...
系统默认**只有你一个账号**。想让朋友注册，在 `.env` 里加一行：

```bash
INVITE_CODE="用 node -e \"console.log(require('crypto').randomBytes(24).toString('base64url'))\" 生成"
```

配了之后 `/signup` 就开放了，朋友填对邀请码才能建号。每个账号是完全独立的租户，
看不到别人家的孩子和数据。想停止新增，把这行删掉或改掉即可（已注册的账号不受影响）。
