...
3. 后台「孩子档案」页每个孩子下面就是**完整的孩子端链接**（形如 `https://你的域名/kid/xxx`），
   点一下在新标签页打开，也可以整条复制。地址是按你当前访问后台用的域名拼的——
   从自定义域名进后台，复制出去的就是自定义域名。
   （也可以直接用 `db:seed` 生成的示例孩子改名字继续用，不需要重新建。）
4. 把那条链接发到孩子会用的设备上，用浏览器的"添加到主屏幕"生成桌面图标，之后孩子直接点图标进入。
