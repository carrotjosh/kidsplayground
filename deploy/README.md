...
> **这份文档是备选方案，当前线上跑的是 Vercel。**
>
> 上面「Vercel 国内也打不开」那条后来查清了更具体的原因：不是 `vercel.app`
> 这个域名被封，是 **Vercel 的 anycast IP 从国内连不上**（换自己的域名直连一样不行）。
> 而 Cloudflare 边缘从国内是通的，所以现在走的是
> **自定义域名 → Cloudflare 橙云代理 → Vercel 回源**，配置见主 README 的「2.1 自定义域名与国内访问」。
>
> 如果那条路最终也走不通（比如运营商线路上 Cloudflare 也不稳），再回来用这份文档。
> 注意：**公司网络那一条它救不了**——Roche 网关按 TLD 封锁（`.xyz` 是 DNS 劫持 + TLS SNI
> 双层拦截），换成 Oracle 自建同样打不开，那只能换一个没被封的 TLD。
