...
import { monthlyBonusPoints } from "@/lib/economy";
...
export async function getMonthSummary(
  ...
  const [tasks, plants, caught, bonusEntry, templates] = await Promise.all([
    ...
    prisma.taskTemplate.findMany({
      ...
      // 只用来判断哪几天排了任务（scheduledTaskDates）。
      // **金额不再从这里算**——满勤奖按达标线走，见下面 bonusPoints。
  ...
  return {
    ...
    // 满勤奖金额按**达标线**算（= lib/economy.ts 的 economyRate），
    // 不再按任务模板分值之和——那是"全做完"的理论上限，蓬蓬的例子里差 3.5 倍。
    bonusPoints: monthlyBonusPoints(child.dailyGoalPoints),
...
export async function settleMonthlyBonusForChild(child: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const templates = await tx.taskTemplate.findMany({
      where: { childId, active: true },
      // 同 getMonthSummary：只用来判断哪几天排了任务，金额不从这里算
      select: { scheduleType: true, weekdays: true },
    });
    // 满勤奖按**达标线**算，见 getMonthSummary 里同样的注释
    const goalPoints = locked.dailyGoalPoints;
    ...
    while (cursor < currentMonth) {
      ...
      if (taskDays > 0 && reachedDays >= Math.ceil(taskDays * MONTHLY_BONUS_RATIO)) {
        // 按结算这一刻的达标线算金额。补发很久以前的月份时用的也是今天的达标线——
        // 这是刻意的：金额一旦写进流水就是快照，与其为历史月份还原当时的任务模板
        // （达标线会被改，还原不出来），不如用一个简单可解释的规则。
        await tx.pointsLedger.create({
          data: {
            childId,
            amount: monthlyBonusPoints(goalPoints),
