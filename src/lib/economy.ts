...
export const AMOUNT_MULTIPLIERS = {
   ...
   * 每集齐 8 种不同宝可梦的里程碑奖励。
   *
   * 原来是 4（日薪 25 时 100、日薪 48 时 192），**那个值把图鉴从消耗口变成了产出口**。
   * 模拟 3000 次拿下一个里程碑的真实成本（含重复返还抵扣）：
   *
   *   已有 0 种 → 净花 152，奖励 192，**净赚 40**
   *   已有 32 种 → 净花 162，净赚 30
   *   已有 88 种 → 净花 193，持平
   *   已有 120 种 → 净花 832，净亏 640
   *
   * 也就是前 11 个里程碑（差不多第一年多）扔球一直是赚的，之后突然够不着——
   * 形状还是反的。真实案例：蓬蓬头 10 天做任务挣 172，一次里程碑就发了 192。
   *
   * 1.5 是从不变量倒推的，不是拍的：里程碑摊到每个球上是 M × p̄ / 8
   * （p̄ = 精灵球的加权平均抓取率 0.366），要 ≤ 球价 8 × MAX_RETURN_RATIO 0.5，
   * 解出 M ≤ 87。取 1.5（日薪 48 时 72，摊每球 3.30）留出余量。
   * 这条现在在 lib/economyAudit.ts 里有实时校验，改大了会被体检拦住。
   *
   * 后来又从 1.5 降到 0.8：1.5（日薪 48 时 72）虽然不再让扔球赚钱，但仍然盖过了
   * 打卡本身——蓬蓬实际每天到手约 17 阳光（日薪 48 是"全部任务都完成"的理论值，
   * 他的完成率约 35%），72 等于四天的真实收入。
   *
   * 0.8 是对着系统里已有的锚点选的，不是又拍一个数：
   *   和「★3 重复返还」同值（0.8）、约等于 5 个精灵球、他两天的真实收入。
   * 是「收集到一个段落」的拍肩，不是一笔收入。
   */
