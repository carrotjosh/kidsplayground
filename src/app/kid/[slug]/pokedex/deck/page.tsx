...
export default async function DeckPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <p className="pixel-card kid-text shrink-0 bg-white p-3 kid-label text-slate-700">
        <Pinyin text={`${deck.length} 种 · 一共 ${owned.length} 只 · 已集满 ${mastered} 种`} />
        <span className="ml-3 kid-label text-slate-500">
          {[4, 3, 2, 1].map((r) => (
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto">
        {deck.length === 0 ? (
          <p className="pixel-card kid-text bg-white p-6 text-center kid-body text-slate-500">
        ...
        {fled.length > 0 && (
          <section className="flex flex-col gap-2">
            <h2 className="pixel-text-outline kid-text kid-label text-white">
