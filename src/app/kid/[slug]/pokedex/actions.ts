...
import { refreshEncounters, throwBall, type ThrowResult } from "@/lib/pokedex";
 ...
 * 参数顺序是为了配合 .bind(null, slug, encounterId, ballTypeId) —— 绑完之后签名正好是
...
export async function throwBallAction(
  ...
  try {
    result = await throwBall(encounterId, ballTypeId, child.id);
  } catch (error) {
...
/** 花阳光刷新今天遇到的宝可梦。结果直接靠 revalidate 刷页面，不用回传状态。 */
export async function refreshEncountersAction(slug: string) {
  const child = await requireChildBySlug(slug);
  await refreshEncounters(child.id, todayDateString());
  revalidatePath(`/kid/${slug}/pokedex`);
}
